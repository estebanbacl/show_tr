...
resource "aws_apigatewayv2_stage" "web_socket_api_stage" {
  api_id        = aws_apigatewayv2_api.web_socket_api.id
  name          = "dev"
...
resource "aws_apigatewayv2_integration" "ws_connect_integration" {
  ...
  integration_uri    = aws_lambda_function.web_socket_connect_lambda.invoke_arn
...
resource "aws_apigatewayv2_integration" "ws_disconnect_integration" {
  ...
  integration_type   = "AWS_PROXY"
  description        = "Lambda example"
  integration_uri    = aws_lambda_function.web_socket_disconnect_lambda.invoke_arn
  integration_method = "POST"
}
